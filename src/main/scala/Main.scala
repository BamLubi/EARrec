...
class Rule(_his: ArrayBuffer[String], _item: String) {
  ...
  def calculate(D: Array[Array[String]]): Unit ={
    val pre: Double = Main.get_frequency(his, D)
    his.append(item)
    val all: Double = Main.get_frequency(his, D)
...
//  val basket_path = "sample/trans_10W.txt"
  val basket_path = "sample/test_2W.txt"
  val user_path = "sample/test_2W.txt"
//  val pattern_path = "out/pattern.txt"
  val pattern_path = "sample/pattern_1W.txt"
...
//    sc.parallelize(pattern).saveAsTextFile("hdfs://127.0.0.1:9000/earrec/out/pattern.txt")
    ...
    if (pattern.isEmpty) {
      ...
      sc.textFile(base_path + pattern_path).collect().foreach(x => {
